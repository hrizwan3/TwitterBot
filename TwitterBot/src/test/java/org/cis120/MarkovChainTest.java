...
public class MarkovChainTest {
    ...
    @Test
    public void testAddBigramNullParam1() {
        MarkovChain mc = new MarkovChain();
        IllegalArgumentException thrown = assertThrows(IllegalArgumentException.class,
                () -> mc.addBigram(null, "null"),
                "addBigram no throw exception :("
        );
        assertTrue(thrown.getMessage().equals("parameters should not be null"));
    }
    ...
    @Test
    public void testTrainEmptyCase() {
        MarkovChain mc = new MarkovChain();
        String sentence = "  ";
        mc.train(Arrays.stream(sentence.split(" ")).iterator());
        mc.reset();
        ...
        String sent = "uno dos tres cuatro";
        mc.train(Arrays.stream(sent.split(" ")).iterator());
        assertEquals(mc.startWords.count("uno"), 1);
        mc.reset();
    ...
    public void testOrderedWalk() {
        String[] expectedWords = {"Never", "gonna", "give", "you", "up"};

        Integer[] walkInd = {1, 0, 1, 0, 0};

        NumberGenerator ng = new ListNumberGenerator(List.of(walkInd));
        MarkovChain mc = new MarkovChain(ng);

        String s1 = "raise you up";
        String s2 = "gonna give you";
        String s3 = "Never gonna";

        mc.train(Arrays.stream(s1.split(" ")).iterator());
        mc.train(Arrays.stream(s2.split(" ")).iterator());
        mc.train(Arrays.stream(s3.split(" ")).iterator());
