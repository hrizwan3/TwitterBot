...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        if (first == null || second == null) {
            ...
            if (chain.containsKey(first)) {
                ...
                ProbabilityDistribution<String> p = new ProbabilityDistribution<>();
                p.record(second);
                chain.put(first, p);
            }
        }
    ...
    public boolean hasNext() {
        if (nextWord.equals(END_TOKEN) || nextWord == null) {
            return false;
        }
        return true;
    }
    ...
    @Override
    public String next() {
        String curr = this.nextWord;
        if (!this.hasNext()) {
            throw new NoSuchElementException("no more words");
        }

        ProbabilityDistribution<String> p = chain.get(curr);

        if (p == null) {
            this.nextWord = null;
            return null;
        }
        this.nextWord = p.pick(ng);
