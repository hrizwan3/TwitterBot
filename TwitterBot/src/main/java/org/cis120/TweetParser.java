...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        ...
        String[] arrOfStr = csvLine.split(",");
        try {
            return arrOfStr[csvColumn];
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        ...
        while (f.hasNext()) {
            String s = extractColumn(f.next(), tweetColumn);
            if (s != null) {
                l.add(s);
            }
        }
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        ...
        String tweetMinusURL = removeURLs(tweet);

        List<String> sentences = tweetSplit(tweetMinusURL);

        for (String s : sentences) {
            if (s != null) {
                out.add(parseAndCleanSentence(s));
            }
        }
        return out;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(
        ...
        for (String t : tweets) {
            List<List<String>> sentence = parseAndCleanTweet(t);
            if (!sentence.isEmpty()) {
                for (List<String> word : sentence) {
                    if (!sentence.isEmpty() && sentence != null) {
                        trainingData.add(word);
                    }
                }
            }
        }
